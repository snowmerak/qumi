...
비발디 시작 페이지나 `chrome://` 같은 브라우저 내부 탭에서는 웹페이지 본문에 연결할 수 없어도 `navigate_to_url`, `list_open_tabs`, `switch_to_tab`을 사용할 수 있습니다. 현재 탭에서 HTTP(S) 주소로 이동하거나 새 탭을 열 수 있고, 같은 창의 내부 탭을 포함해 기존 탭으로 전환할 수 있습니다. 본문 읽기·쓰기와 키 입력 도구는 일반 HTTP(S) 페이지가 로드되고 사이트 접근 권한을 받은 뒤에만 제공됩니다. 이동 후 새 페이지의 본문 도구는 로딩과 재연결이 끝난 다음 요청부터 사용할 수 있습니다.
