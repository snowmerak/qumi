...
연결된 웹페이지에서 문장을 드래그한 뒤 사이드 패널의 **+ 선택 영역 추가**를 누르면 CSS 선택자, 선택한 텍스트, 해당 범위의 HTML 조각이 질문에 첨부됩니다. 한 요청에 최대 5개를 추가하거나 보내기 전에 각각 제거할 수 있습니다. 예를 들어 "한국어로 번역해 줘", "이 문체로 다시 써 줘", "문법을 검사하고 수정해 줘"라고 요청할 수 있습니다. 첨부 내용은 대화 내역에 작은 `<주석 1>` 표시로 남고, 마우스를 올리거나 눌러 펼칠 수 있습니다. 선택한 텍스트는 최대 16,000자, HTML은 최대 24,000자까지 포함하며 넘으면 주석에 잘림이 표시됩니다. 첨부한 페이지 내용은 모델 요청과 확장 로컬 대화 기록에 저장됩니다.
