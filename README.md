...
실행을 분석하려면 설정의 **실행 진단 로그 → JSON 다운로드**를 사용하세요. 각 대화 턴의 모델 요청, 도구 호출, 압축, 오류와 소요 시간·토큰 수를 `turnId`와 순번으로 묶어 기록합니다. 로그는 Chrome의 확장 로컬 저장소(`qumiExecutionLog`)에 최근 7일, 최대 2,000개 이벤트까지 보관합니다. 프롬프트, 페이지 내용, 도구 인수 값·결과, API 키와 원본 오류 메시지는 기록하지 않습니다. 설정에서 로그를 지울 수 있습니다.
