...
const messages = {
  ...
  mcpHint: { en: "Register a Streamable HTTP endpoint with request headers or OAuth. MCP tool calls use the change confirmation setting.", ko: "Streamable HTTP 주소를 요청 헤더 또는 OAuth로 등록합니다. MCP 도구 호출에는 변경 작업 확인 설정이 적용됩니다.", ja: "Streamable HTTP エンドポイントをリクエストヘッダーまたは OAuth で登録します。MCP ツールには変更操作の確認設定が適用されます。", zh: "使用请求头或 OAuth 注册 Streamable HTTP 端点。MCP 工具调用遵循更改确认设置。" },
  ...
  mcpOAuthHint: { en: "Use a registered client ID, a public metadata URL, or leave both empty for dynamic registration. After adding, select Sign in, then save settings. Tokens last for this browser session.", ko: "등록된 클라이언트 ID나 공개 메타데이터 URL을 입력하세요. 둘 다 비우면 동적 등록을 시도합니다. 추가 후 로그인하고 설정을 저장하세요. 토큰은 브라우저 세션 동안 유지됩니다.", ja: "登録済みクライアント ID または公開メタデータ URL を入力します。両方空欄なら動的登録を試みます。追加後にログインして設定を保存してください。トークンはブラウザーセッション中保持されます。", zh: "输入已注册的客户端 ID 或公开元数据 URL；两者留空则尝试动态注册。添加后登录并保存设置。令牌仅在本次浏览器会话中保留。" },
